...
class TestVerifyInternalToken:
    def test_correct_token_passes(self):
        with patch("app.api.deps.settings.INTERNAL_API_TOKEN", "correct-secret"):
    ...
    def test_unset_server_token_rejects_every_request(self):
        ...
        with patch("app.api.deps.settings.INTERNAL_API_TOKEN", ""):
            with pytest.raises(HTTPException) as exc_info:
    ...
    def test_uses_constant_time_comparison_not_plain_equality(self):
        ...
        with patch("app.api.deps.settings.INTERNAL_API_TOKEN", "correct-secret"), \
             patch("app.api.deps.secrets.compare_digest", return_value=True) as mock_compare:
    ...
    def test_correct_length_wrong_content_still_rejected(self):
        ...
        with patch("app.api.deps.settings.INTERNAL_API_TOKEN", "abcdefgh"):
            with pytest.raises(HTTPException) as exc_info:
...
class TestNonAsciiTokenHandling:
    ...
    def test_non_ascii_configured_secret_does_not_crash(self):
        ...
        with patch("app.api.deps.settings.INTERNAL_API_TOKEN", "café-secret"):
            with pytest.raises(HTTPException) as exc_info:
